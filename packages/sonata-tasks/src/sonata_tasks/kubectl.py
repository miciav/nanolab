...
def k8s_function_resources_absent(
    *,
    ...
    """Wait until the function's Deployment and Service have both disappeared."""
    attempts = max(1, timeout_seconds // 2)
    resources = (f"deployment/fn-{function}", f"service/fn-{function}")
    checks = " ".join(
        f"kubectl -n {shlex.quote(namespace)} get {shlex.quote(resource)} >/dev/null 2>&1 && present=1;"
