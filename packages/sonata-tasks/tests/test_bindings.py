...
def test_a_spec_without_a_role_runs_on_the_host() -> None:
    """Roleless specs exist: the provisioning operations build them, and the
    host is the only place they could mean."""
    host = _RecordingExecutor([])
    stack = _RecordingExecutor([])
    executor = RoleBoundCommandTaskExecutor(RoleBindings(host=host, stack=stack))
    host_task = CommandTaskSpec(task_id="host", summary="Host", argv=("true",))
    stack_task = CommandTaskSpec(task_id="stack", summary="Stack", argv=("true",), role="stack")
